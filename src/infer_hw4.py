"""Create the official HW4 pred.npz submission file.

The exporter preserves the original test filenames and output sizes. It does not
crop test images; it pads internally to satisfy PromptIR's downsampling multiple
and unpads before saving.
"""
...
from utils.dataset_utils import HW4TestDataset
...
def parse_args():
    ...
    parser.add_argument(
        "--rgb_calibration_json",
        type=str,
        default="",
        help="Optional calibration JSON from tools/rgb_calibration.py.",
    )
    parser.add_argument(
        "--chroma_gamma",
        type=float,
        default=1.0,
        help="Shrink YCbCr chroma residuals using degraded input as anchor.",
    )
    return parser.parse_args()


def load_rgb_calibration(path, device):
    if not path:
        return None
    payload = json.loads(Path(path).read_text(encoding="utf-8"))
    gains = torch.tensor(payload["gain_rgb"], device=device, dtype=torch.float32)
    biases = torch.tensor(payload["bias_rgb"], device=device, dtype=torch.float32)
    return gains.view(1, 3, 1, 1), biases.view(1, 3, 1, 1)


def apply_rgb_calibration(tensor, calibration):
    if calibration is None:
        return tensor
    gains, biases = calibration
    gains = gains.to(device=tensor.device, dtype=tensor.dtype)
    biases = biases.to(device=tensor.device, dtype=tensor.dtype)
...
def main():
    ...
    print("HW4 submission inference")
    print(f"  test_dir: {Path(args.test_dir)}")
    print(f"  checkpoint: {Path(args.ckpt_path)}")
    print(f"  output_npz: {Path(args.output_npz)}")
    print(f"  device: {device}")
    print(f"  EMA enabled: {not args.disable_ema}")
    print(f"  x8 TTA: {args.tta}")
    print(
        "  Gaussian patch: "
        f"{args.gaussian_patch} "
        f"(tile_size={args.tile_size}, patch_grid={args.patch_grid}, "
        f"patch_batch_size={args.patch_batch_size})"
    )
    print(
        "  tiled inference: "
        f"{args.tile} (tile_size={args.tile_size}, overlap={args.tile_overlap})"
    )
    if args.rgb_calibration_json:
        print(f"  RGB calibration JSON: {args.rgb_calibration_json}")
    if abs(float(args.chroma_gamma) - 1.0) > 1e-12:
        print(f"  chroma gamma: {args.chroma_gamma:g}")
    rgb_calibration = load_rgb_calibration(args.rgb_calibration_json, device)
    if rgb_calibration is not None:
        gains, biases = rgb_calibration
        print(
            "Using RGB calibration: "
            f"gain={gains.flatten().tolist()}, "
            f"bias_pixel={(biases.flatten() * 255.0).tolist()}"
        )
    if abs(float(args.chroma_gamma) - 1.0) > 1e-12:
        print(f"Using chroma residual shrinkage: gamma={args.chroma_gamma:g}")
    ...
    dataset = HW4TestDataset(args.test_dir)
    loader = DataLoader(dataset, batch_size=1, shuffle=False, num_workers=0)
    predictions = {}
    input_sizes = {}

    with torch.no_grad():
        for metadata, degraded in tqdm(loader, desc="HW4 inference"):
            filename = metadata["filename"][0]
            height = int(metadata["height"].item())
            width = int(metadata["width"].item())
            ...
            if abs(float(args.chroma_gamma) - 1.0) > 1e-12:
                restored = shrink_chroma_residual(
                    restored,
                    degraded,
                    gamma=args.chroma_gamma,
                )
            restored = apply_rgb_calibration(restored, rgb_calibration)
            output = tensor_to_uint8_chw(restored)
            if output.shape[1:] != (height, width):
                raise RuntimeError(
                    f"Shape mismatch for {filename}: expected {(3, height, width)}, "
                    f"got {output.shape}."
                )
            predictions[filename] = output
            input_sizes[filename] = (height, width)
    ...
    print(f"Saved {len(predictions)} predictions to {Path(args.output_npz).resolve()}")
