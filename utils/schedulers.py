...
class LinearWarmupCosineAnnealingLR(_LRScheduler):
    ...
    def get_lr(self) -> List[float]:
        ...
        if self.warmup_epochs <= 0:
            cosine_denominator = max(1, self.max_epochs)
            return [
                self.eta_min
                + 0.5
                * (base_lr - self.eta_min)
                * (1 + math.cos(math.pi * self.last_epoch / cosine_denominator))
                for base_lr in self.base_lrs
            ]
        if self.last_epoch == 0:
            return [self.warmup_start_lr] * len(self.base_lrs)
        if self.last_epoch < self.warmup_epochs:
            warmup_denominator = max(1, self.warmup_epochs - 1)
            return [
                group["lr"] + (base_lr - self.warmup_start_lr) / warmup_denominator
                for base_lr, group in zip(self.base_lrs, self.optimizer.param_groups)
            ]
        if self.last_epoch == self.warmup_epochs:
            return self.base_lrs
        cosine_denominator = max(1, self.max_epochs - self.warmup_epochs)
        if (self.last_epoch - 1 - self.max_epochs) % (2 * cosine_denominator) == 0:
            return [
                group["lr"]
                + (base_lr - self.eta_min) * (1 - math.cos(math.pi / cosine_denominator)) / 2
                for base_lr, group in zip(self.base_lrs, self.optimizer.param_groups)
            ]

        return [
            (1 + math.cos(math.pi * (self.last_epoch - self.warmup_epochs) / cosine_denominator))
            / (
                1
                + math.cos(
                    math.pi * (self.last_epoch - self.warmup_epochs - 1) / cosine_denominator
                )
            )
    ...
    def _get_closed_form_lr(self) -> List[float]:
        ...
        if self.last_epoch < self.warmup_epochs:
            warmup_denominator = max(1, self.warmup_epochs - 1)
            return [
                self.warmup_start_lr + self.last_epoch * (base_lr - self.warmup_start_lr) / warmup_denominator
                for base_lr in self.base_lrs
            ]
